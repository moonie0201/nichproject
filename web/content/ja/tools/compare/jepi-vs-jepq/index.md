---
title: "JEPI vs JEPQ — ETF 比較"
description: "JEPIとJEPQの並列比較：配当利回り・経費率・1年・5年リターン・リスクメモ。任意の投資額から年間インカムを試算できます。"
date: 2026-06-14T00:00:00+09:00
lastmod: 2026-06-14T00:00:00+09:00
draft: false
type: "tools"
tool: "compare"
ticker_a: "JEPI"
ticker_b: "JEPQ"
schema: "Article"
author: "InvestIQs Editorial"
tags: ["JEPI", "JEPQ", "ETF比較", "配当ETF"]
data_as_of: "2026-06-14"
disclaimer: true
tickers: [JEPI, JEPQ]
---

## JEPI vs JEPQ: 主な違い

JEPIとJEPQはJPモルガンの同じエクイティ・プレミアム・インカム戦略を使いますが、株式基盤が異なります。JEPIは低ボラティリティの米国株ポートフォリオを使い、JEPQはナスダック100型の保有銘柄に同じオーバーレイを適用します。

JEPQはナスダックのハイテク比率が高い分、成長ポテンシャルとボラティリティの両方が高くなります。JEPIの低ボラティリティ株選択は、歴史的により安定したリターン推移をもたらしています。

## インカムを試算する

上の投資額を入力して、JEPIとJEPQの年間・月間インカム推計を並べて比較してください。

## 関連ツール

- [JEPI dividend calculator](/ja/tools/dividend-calculator/jepi/)
- [JEPQ dividend calculator](/ja/tools/dividend-calculator/jepq/)
- [関連ツール](/ja/tools/)

毎週金曜、配当ETFと米国市場のデータをまとめたニュースレターをお届けします。
