---
title: "JEPQ 配当金 計算機 — 配当再投資シミュレーション"
description: "JEPQ(JPMorgan Nasdaq Equity Premium Income ETF)の配当利回りをもとに、投資額・保有年数・増配率・配当再投資から将来の配当金と評価額を試算できる計算機です。円建て表示にも対応。"
date: 2026-06-14T00:00:00+09:00
lastmod: 2026-06-14T00:00:00+09:00
draft: false
type: "tools"
tool: "dividend-calculator"
ticker: "JEPQ"
schema: "Article"
author: "InvestIQs Editorial"
tags: ["JEPQ", "配当ETF", "新NISA", "配当再投資"]
data_as_of: "2026-06-14"
disclaimer: true
---

## JEPQ の配当金を数字で確認する

JEPQ(JPMorgan Nasdaq Equity Premium Income ETF)は、JEPIと同じ「エクイティ・プレミアム・インカム」戦略を、ナスダック100型のポートフォリオに適用したものです。成長エクスポージャーが高い分、利回りもボラティリティも高くなります。

## 配当利回りだけで判断しない

JEPQはハイテク比率が高いためJEPIより変動が大きく、設定が2022年と運用期間が短いため、長期の数値は参考値として扱う必要があります。

数値は定期的に更新される参考値であり、リアルタイムではありません。実際の配当は変動し、税金や為替の影響も受けます。

## 関連ツール・記事

- 他の配当ETFと比較する:
  - [SCHD](/ja/tools/dividend-calculator/schd/)
  - [JEPI](/ja/tools/dividend-calculator/jepi/)
- 投資の基礎を学ぶ: [投資の学び](/ja/study/)

毎週金曜、配当ETFと米国市場のデータをまとめたニュースレターをお届けします。
