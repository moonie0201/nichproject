---
title: "SCHD 配当金 計算機 — 配当再投資シミュレーション"
description: "SCHD(Schwab U.S. Dividend Equity ETF)の配当利回りをもとに、投資額・保有年数・増配率・配当再投資から将来の配当金と評価額を試算できる計算機です。円建て表示にも対応。"
date: 2026-06-14T00:00:00+09:00
lastmod: 2026-06-14T00:00:00+09:00
draft: false
type: "tools"
tool: "dividend-calculator"
ticker: "SCHD"
schema: "Article"
author: "InvestIQs Editorial"
tags: ["SCHD", "配当ETF", "新NISA", "配当再投資"]
data_as_of: "2026-06-14"
disclaimer: true
tickers: [SCHD]
---

## SCHD の配当金を数字で確認する

SCHD(Schwab U.S. Dividend Equity ETF)は、増配を継続してきた質の高い米国企業に、非常に低い経費率で分散投資するETFです。表面利回りの高さよりも「増配」と財務の質を重視します。

## 配当利回りだけで判断しない

SCHDは質と増配でスクリーニングするため利回りは中程度です。その代わり、配当を増やし続けられる企業に焦点を当てており、高分配のカバードコール型とは値動きの性質が異なります。

数値は定期的に更新される参考値であり、リアルタイムではありません。実際の配当は変動し、税金や為替の影響も受けます。

## 関連ツール・記事

- 他の配当ETFと比較する:
  - [JEPI](/ja/tools/dividend-calculator/jepi/)
  - [VYM](/ja/tools/dividend-calculator/vym/)
- 投資の基礎を学ぶ: [投資の学び](/ja/study/)

毎週金曜、配当ETFと米国市場のデータをまとめたニュースレターをお届けします。
